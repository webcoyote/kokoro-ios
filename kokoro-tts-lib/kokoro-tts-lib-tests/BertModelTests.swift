...
struct BertModelTests {
    @Test func testBertModel() async throws {
        ...
        let model = try BertModel()
        let output = try model.forwardPassOnInitialModel(tokens: tokens)
        let _ = try model.forwardPassOnEncoderModel(array: output)
    }
}
