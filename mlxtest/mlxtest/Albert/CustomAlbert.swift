...
class CustomAlbert {
  ...
  init(weights: [String: MLXArray], config: AlbertModelArgs) {
    self.config = config
    self.embeddings = AlbertEmbeddings(weights: weights, config: config)
    self.encoder = AlbertEncoder(weights: weights, config: config)
    self.pooler = Linear(weight: weights["bert.pooler.weight"]!, bias: weights["bert.pooler.bias"]!)
  }
  ...
  func callAsFunction(
    ...
    var attentionMaskProcessed: MLXArray? = nil
