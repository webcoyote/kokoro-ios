...
class AlbertEmbeddings {
    ...
    init(weights: [String: MLXArray], config: AlbertModelArgs) {
        self.wordEmbeddings = Embedding(weight: weights["bert.embeddings.word_embeddings.weight"]!)
        self.positionEmbeddings = Embedding(weight: weights["bert.embeddings.position_embeddings.weight"]!)
        self.tokenTypeEmbeddings = Embedding(weight: weights["bert.embeddings.token_type_embeddings.weight"]!)
    ...
    func callAsFunction(
        ...
        if let positionIds = positionIds {
            ...
            positionIdsUsed = MLX.expandedDimensions(MLXArray.init(0..<seqLength), axes: [0])
        }
